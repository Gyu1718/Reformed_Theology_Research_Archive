...
(function () {
  ...
  function volumeOf(ref) {
    var m = (ref || "").match(/^CD\s+([IV]+\/\d|IV\/4)/);
    return m ? m[1] : "";
  }
  function inferVolume(partTitle) {
    if (/제9장/.test(partTitle)) return "III/1";
    if (/제10장/.test(partTitle)) return "III/2";
    if (/제11장/.test(partTitle)) return "III/3";
    if (/제12장/.test(partTitle)) return "III/4";
    if (/제13장|제14장/.test(partTitle)) return "IV/1";
    if (/제15장/.test(partTitle)) return "IV/2";
    if (/제16장/.test(partTitle)) return "IV/3";
    if (/단편|세례/.test(partTitle)) return "IV/4";
    return "";
  }
  function enhancedDetail(ref, title, points) {
    ...
    return base + pointText;
  }

  function sectionToChapter(section, volumeId) {
    if (typeof section === "string") {
      var match = section.match(/^(§\d+)\s*(.*)$/);
      var sec = match ? match[1] : "";
      var title = match ? match[2] : section;
      var ref = sec && volumeId ? "CD " + volumeId + " " + sec : section;
      var points = title ? [title] : [];
  ...
  function partFromMap(volume, part) {
    ...
    var inferred = inferVolume(part.largeTopic || "") || volume.volume;
  ...
  function attachQuotesToBooks(books, quotePacks) {
    var allQuotes = [];
    quotePacks.forEach(function (pack) { if (pack && Array.isArray(pack.quotes)) allQuotes = allQuotes.concat(pack.quotes); });
    ...
    books.forEach(function (book) {
      ...
      book.parts.forEach(function (part) {
        (part.chapters || []).forEach(function (chapter) {
          ...
          var seen = {};
          chapter.quotes = merged.filter(function (item) { var key = [item.text, item.ref].join("|"); if (seen[key]) return false; seen[key] = true; return true; });
        });
      });
    });
    return books;
  ...
  var quotePacks = [loadJson("./data/quotes/barth-translated-sentence-quotes-v1.json", null)].filter(Boolean);

  window.__DATA__ = { books: attachQuotesToBooks(combinedBooks, quotePacks), authors: loadJson("./data/authors.json", []), topics: loadJson("./data/topics.json", []), passages: loadJson("./data/passages.json", []), notes: loadJson("./data/notes.json", []), taxonomy: loadJson("./data/taxonomy.json", {}) };
})();
